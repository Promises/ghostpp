...
void CBaseGame :: loop( )
{
	...
	if( m_Replay && ( m_GameLoading || m_GameLoaded ) )
	{
		...
		m_Replay->BuildReplay( m_GameName, m_StatString, m_GHost->m_ReplayWar3Version, m_GHost->m_ReplayBuildNumber, false );
		...
		m_Replay->BuildReplay( m_GameName, m_StatString, m_GHost->m_ReplayWar3Version, m_GHost->m_ReplayBuildNumber, true );
		m_Replay->Save( m_GHost->m_TFT, m_GHost->m_ReplayPath + UTIL_FileSafeName( m_UUID + ".w3g" ) );
	}
...
{
	if(m_GHost->m_WS)
		m_GHost->m_WS->SendGameAction(WSGameAction::MESSAGE, m_GameName, message);
...
CGamePlayer *CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer, double *score )
{
	...
	if(m_GHost->m_WS) {
		...
		m_GHost->m_WS->SendPlayerAction(WSPlayerAction::JOINED, m_GameName, Player->GetName( ), JoinedRealm, SID, "");
	}
...
void CBaseGame :: EventPlayerLeft( CGamePlayer *player, uint32_t reason )
{
	if(m_GHost->m_WS) {
		...
		m_GHost->m_WS->SendPlayerAction(WSPlayerAction::LEFT, m_GameName, player->GetName( ), Realm, GetSIDFromPID(player->GetPID()), "");
	}
				...
				else if( ExtraFlags[0] == 0 )
				{
					...
					if( m_Tournament && m_TournamentChatID != 0 )
					{
						...
						if(false && m_GHost->m_WS) {
							...
							m_GHost->m_WS->SendPlayerAction(WSPlayerAction::MESSAGE, m_GameName, player->GetName(), Realm, slot, chatPlayer->GetMessage());
						}
					}
...
void CBaseGame :: EventPlayerChangeTeam( CGamePlayer *player, unsigned char team )
{
	...
	if( m_Map->GetMapOptions( ) & MAPOPT_CUSTOMFORCES )
	{
		...
		if(m_GHost->m_WS && newSID < m_Slots.size( )) {
        	CGamePlayer *newplayer = GetPlayerFromSID(newSID);

			string Realm = newplayer->GetSpoofedRealm( );

			if( !newplayer->GetSpoofed( ) )
				Realm = newplayer->GetJoinedRealm( );

        	m_GHost->m_WS->SendPlayerAction(WSPlayerAction::SLOT_CHANGED, m_GameName, newplayer->GetName(), newplayer->GetSpoofedRealm(), newSID, "");
		}
	}
	else
	{
