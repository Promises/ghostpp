...
{
    if(m_GHost->m_WS)
	    m_GHost->m_WS->SendGameAction(WSGameAction::CREATE, m_GameName, UTIL_ToString(GetSlotsOpen()));
...
uint32_t CGame :: getExperienceAtLevel(uint32_t level)
{
    double points = 0;
...
uint32_t CGame :: getLevelAtExperience(uint32_t experience)
{
...
void CGame :: EventPlayerDeleted( CGamePlayer *player )
{
	...
	if( m_GameLoading || m_GameLoaded )
	{
		...
		if( !m_SoftGameOver && !m_MapType.empty( ) && m_Stats && m_GameOverTime == 0 && !m_Stats->IsWinner( ) && Team != 24 && m_NumTeams == 2 && !m_SoloTeam && m_GameTicks < DrawTicks && m_StartPlayers > 6 && m_MapType != "treetag" && m_MapType != "battleships"  )
		{
			...
			while( n < PacketLength && !Failed )
			{
				...
				switch ( CurrentID )
				{
                        ...
                        case 0x50 :
                            {
                              n += 6;
                              player->IncrementShareActions( );

                              if( player->GetShareActions( ) > 500 )
                              {
                                CONSOLE_Print( "[GAME: " + m_GameName + "] warning: detected laghack" );
                                SendAllChat( "Laghack detected!" );
                                SendAllChat( "Player [" + player->GetName( ) + "] was prevented from laghacking." );

                                m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( player->GetSpoofedRealm(), player->GetName( ), player->GetExternalIPString(), m_GameName, "antilaghack", "Laghacking tool detected in game.", 3600 * 24 * 30 * 12, "" ));

                                player->SetDeleteMe( true );
                                player->SetLeftReason( "was kicked by anti-laghack" );
						...
						case 0x51 :
						{
							...
							if( m_MapType == "dota" || m_MapType == "dota2" || m_MapType == "dotaab" || m_MapType == "lod" || m_MapType == "eihl" || m_MapType == "lodab" )
							{
                                ...
                                m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( player->GetSpoofedRealm(), player->GetName( ), player->GetExternalIPString(), m_GameName, "antitradehack", "Trade hacking tool detected in game.", 3600 * 24 * 30 * 12, "" ));
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	*********************/
    ...
    if( (Command == "votestart" || Command == "vs") && !m_CountDownStarted && (votestartAuth || votestartAutohost || !m_GHost->m_VoteStartAutohostOnly) && !player->GetMuted( ) )
	{
...
void CGame :: EventGameStarted( )
{
    ...
    if(m_GHost->m_WS)
	    m_GHost->m_WS->SendGameAction(WSGameAction::STARTED, m_GameName, "");
...
void CGame :: SaveGameData( )
{
	...
	m_CallableGameAdd = m_GHost->m_DB->ThreadedGameAdd( m_GHost->m_BNETs.size( ) == 1 ? m_GHost->m_BNETs[0]->GetServer( ) : string( ), m_DBGame->GetMap( ), m_GameName, m_OwnerName, m_GameTicks / 1000, m_GameState, m_CreatorName, m_CreatorServer, m_Tournament ? "uxtourney" : string( ), m_LobbyChatEvents, m_GameChatEvents, m_UUID );
}
